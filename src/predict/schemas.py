...
class PredictionRequest(BaseModel):
    datetime: Annotated[
        datetime,
        Field(..., example="2024-01-01T12:00:00", description="Prediction datetime"),
    ]
    ...
    kwh_price: Optional[float] = Field(
        None, example=0.15, description="Price per kWh in selected currency"
    )
...
class PredictionResponse(BaseModel):
    ...
    money_saved: Optional[float] = Field(
        None, example=0.5, description="Money saved in selected currency"
    )
...
class TimeSeriesPredictionRequest(BaseModel):
    start: Annotated[
        datetime,
        Field(..., example="2024-01-01T12:00:00", description="Start datetime"),
    ]
    end: Annotated[
        datetime, Field(..., example="2024-01-01T12:00:00", description="End datetime")
    ]
    kwp: float = Field(..., example=5.0, description="Installed capacity in kW")
    latitude: float = Field(..., example=51.5074, description="Latitude")
    longitude: float = Field(..., example=-0.1278, description="Longitude")
...
class FeatureInput(BaseModel):
    ...
    poa: float = Field(
        ..., example=800.5, description="W/m² (Plane of Array irradiance)"
    )
    clearsky_index: float = Field(..., example=0.8, description="0-2 range")
    cloud_cover_3_moving_average: float = Field(
        ..., example=0.5, description="0-100 range"
    )
    ...
    day_of_year_sin: float = Field(
        ..., example=0.75, description="Encoded day of year cyclically"
    )
    ...
    physical_model_prediction: float = Field(
        ..., example=4.7, description="Physical model prediction in kW"
    )
