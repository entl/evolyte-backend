...
from src.predict.schemas import (PredictionRequest, PredictionResponse, PredictionClientRequest, FeatureInput,
                                 BatchPredictionClientRequest, BatchPredictionClientResponse,
                                 TimeSeriesPredictionRequest, BatchPredictionRequest, BatchPredictionResponse)
...
class PredictionService:
    ...
    def predict(self, request: PredictionRequest) -> PredictionResponse:
        weather_schema = WeatherRequest(latitude=request.latitude, longitude=request.longitude,
                                        start_date=request.datetime.date(), end_date=request.datetime.date(),
                                        azimuth=request.azimuth, tilt=request.tilt)
        ...
        solar_position = self.__calculate_solar_position(latitude=request.latitude, longitude=request.longitude,
                                                         time=request.datetime)
        poa = self.__calculate_poa(weather_data_hourly=weather_data_hourly, tilt=request.tilt, azimuth=request.azimuth,
        ...
        physical_model_prediction = self.__calculate_physical_model(poa=poa,
                                                                    cell_temp=cell_temp,
                                                                    kwp=request.kwp)
        ...
        clear_sky_index = self.__calculate_clear_sky_index(weather_data_hourly=weather_data_hourly,
        ...
        return PredictionResponse(datetime=prediction_response.datetime, prediction=prediction_response.prediction,
                                  co2_saved=self.__calculate_co2_saved(prediction_response.prediction), money_saved=money_saved)
    ...
    def predict_time_series(self, request: TimeSeriesPredictionRequest) -> BatchPredictionResponse:
        ...
        for entry in self.__generate_hourly_records(start_time, end_time):
            # calculate derived features
            weather_data_hourly = reduce(lambda x, y: x if x.time == entry.strftime("%Y-%m-%dT%H:%M") else y,
                                         weather_data.hourly)
            solar_position = self.__calculate_solar_position(latitude=request.latitude, longitude=request.longitude,
                                                             time=entry)

            poa = self.__calculate_poa(weather_data_hourly=weather_data_hourly, tilt=request.tilt,
            ...
            day_of_year_encoding = self.__calculate_cycling_encoding(value=entry.timetuple().tm_yday,
                                                                     period=365)
    ...
    def __calculate_poa(self, weather_data_hourly: HourlyWeatherData, tilt, azimuth, solar_zenith, solar_azimuth):
        # Calculate POA
