...
class SolarPanelService:
    ...
    def get_solar_panels_by_status(
        self, status: PanelStatusEnum
    ...
    def get_solar_panels_based_on_zoom(
        ...
        if zoom_level > 12:
            return self.uow.solar_panels.get_panels_in_bounds(
                min_lat, max_lat, min_lon, max_lon
            )

        grid_size = 0.1 if zoom_level < 5 else 0.01 if zoom_level < 10 else 0.001
        return self.uow.solar_panels.get_clustered_panels(
            min_lat, max_lat, min_lon, max_lon, grid_size
        )

    def get_nearby_solar_panels(
    ...
    def get_clustered_panels(
        ...
        cluster_models = [
            self._solar_panels_cluster_tuple_to_model(cluster) for cluster in clusters
        ]
    ...
    def get_solar_panel_in_bounds(
        ...
        panels = self.uow.solar_panels.get_panels_in_bounds(
            min_lat, max_lat, min_lon, max_lon
        )
        for panel in panels:
            panel.location = self.__wkbelement_to_lat_lon(panel.location)
    ...
    def create_solar_panel(
        self, solar_panel_data: SolarPanelCreate
    ) -> SolarPanelResponse:
        with self.uow as uow:
            ...
            uow.expunge(
                created_solar_panel
            )  # remove from session to avoid auto-update in database
            created_solar_panel.location = self.__wkbelement_to_lat_lon(
                created_solar_panel.location
            )

            return SolarPanelResponse.model_validate(created_solar_panel)

    def create_bulk_solar_panels(
        self, solar_panels: list[SolarPanelCreate]
    ) -> list[SolarPanelResponse]:
        with self.uow:
            ...
            return [
                SolarPanelResponse.model_validate(panel)
                for panel in created_solar_panels
            ]

    def update_solar_panel(
        self, solar_panel_data: SolarPanelUpdate
    ) -> SolarPanelResponse:
        with self.uow:
            ...
            update_data = solar_panel_data.model_dump(
    ...
    def _solar_panels_cluster_tuple_to_model(self, cluster_tuple) -> SolarPanelsCluster:
        return SolarPanelsCluster(
            count=cluster_tuple[0],
            longitude=cluster_tuple[1],
            latitude=cluster_tuple[2],
            min_longitude=cluster_tuple[3],
            max_longitude=cluster_tuple[4],
            min_latitude=cluster_tuple[5],
            max_latitude=cluster_tuple[6],
        )
